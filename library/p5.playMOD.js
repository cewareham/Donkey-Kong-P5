...
function SpriteSheet(pInst) {
  ...
  // draw frame using json file generated by Free Texture Packer WITH MODS!
  this.drawFrameFree = function(frame_name, x, y, width, height) {
    var frameToDraw;
    if (typeof frame_name === 'number') {
      frameToDraw = this.frames[frame_name].frame;
    } else {
      for (var i = 0; i < this.frames.length; i++) {
        if (this.frames[i].filename === frame_name) {
          frameToDraw = this.frames[i].frame;
          break;
        }
      }
    }
    var dWidth = width || frameToDraw.w;
    var dHeight = height || frameToDraw.h;
    pInst.image(this.image, x,                         y,        dWidth,       dHeight,
                            frameToDraw.x, frameToDraw.y, frameToDraw.w, frameToDraw.h);
    // return drawn location & drawn width/height so we can space the next char if necessary
    return { "x" : x, "y" : y, "width": dWidth, "height": dHeight };
}
  ...
  this.drawFrame = function(frame_name, x, y, width, height) {
    ...
    pInst.image(this.image, x,                         y,            dWidth,            dHeight,
                            frameToDraw.x, frameToDraw.y, frameToDraw.width, frameToDraw.height);
  };
